  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      ...
      await api(`/products/${editing.id}/price`, {
        method: 'PATCH',
        body: JSON.stringify({ storeId, price: editForm.price, supplierCost: editForm.supplierCost, deliveryFee: editForm.deliveryFee }),
      }, getToken());
      setEditing(null);
      load();
    } catch (err) {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Table>
          <thead className="bg-slate-50 text-left">
            <tr><th className="p-3">Produto</th><th className="p-3">Preço</th><th className="p-3">Custo</th><th className="p-3">Taxa entrega</th><th className="p-3">Disponível</th><th className="p-3" /></tr>
          </thead>
          <tbody>
            {products.map((p) => (
