...
import { formatCnpj, formatCurrency, cn } from '@/lib/utils';
import { getSaleAttendantName, formatSaleDateLabel } from '@gas-erp/shared';
...
/** Conteúdo do comprovante (barra de ações + cupom). Reutilizável em drawer/página. */
export function SaleReceiptContent({ saleId }: { storeId?: string; saleId: string }) {
  ...
  if (!sale) {
    return <p className="p-6 text-center text-sm text-slate-400">Carregando comprovante…</p>;
  }
...
/** Drawer sobreposto (fundo sombreado) com o comprovante da venda. */
    ...
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      >
        <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <h2 className="text-base font-semibold text-slate-900">Comprovante</h2>
          <button
            type="button"
            onClick={handleClose}
