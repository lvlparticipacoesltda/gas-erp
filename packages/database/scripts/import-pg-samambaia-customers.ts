/**
 * Importa clientes da planilha legada de Praia Grande Samambaia para o Gas ERP.
 *
 * Uso:
 *   pnpm --filter @gas-erp/database import:pgsmb-customers -- --dry-run
 *   pnpm --filter @gas-erp/database import:pgsmb-customers
 *
 * Variáveis (opcionais):
...
import { loadPgSamambaiaCustomersFromCsv } from './lib/pg-samambaia-csv';

const DEFAULT_ORGANIZATION_ID = 'cmqs3fqt20000jkirsp9hvdwv';
const DEFAULT_STORE_ID = 'cmr4rzjnr003lqm01uwheikq5';
const DEFAULT_CSV_PATH = path.resolve(
  __dirname,
  '../../../docs/Clientes_PG1_Consolidado_2024_2026.csv',
);
...
async function main() {
  ...
  const { rawRows, parsedRows, skippedRows, customers } =
    await loadPgSamambaiaCustomersFromCsv(csvPath);
  ...
  try {
    ...
    for (const row of existingRows) {
      const match = (row.notes ?? '').match(/legacy:pgsmb:[^\s]+/);
