...
export const SANTOS_PERIOD_ORDER: Record<string, number> = {
  'De 01/01/2024 até 31/12/2024': 1,
  'De 01/01/2025 até 31/12/2025': 2,
  'De 01/01/2026 até 12/07/2026': 3,
};

export type SantosCsvRow = {
  periodStart: string;
  periodEnd: string;
  periodLabel: string;
  city: string;
...
function parseRowLineAnchored(line: string): string[] | null {
  ...
  const cityMatch = tail.match(/^Santos,(.+)$/);
...
export function buildLegacyKey(name: string, phone: string | null, address: ParsedCustomerAddress): string {
  ...
  return `legacy:sts:nameaddr:${hash}`;
}

function rowFromFields(fields: string[]): SantosCsvRow | null {
...
export function normalizeRow(row: SantosCsvRow): NormalizedCustomer | null {
  ...
  return {
    ...
    name,
    phone,
    notes: `Migrado CSV Santos (2024-2026). ${legacyKey}`,
...
export async function loadSantosCustomersFromCsv(filePath: string): Promise<{
  rawRows: number;
  parsedRows: number;
  skippedRows: number;
