...
export const PG_SAMAMBAIA_PERIOD_ORDER: Record<string, number> = {
  'De 01/01/2024 até 31/12/2024': 1,
  'De 01/01/2025 até 31/12/2025': 2,
  'De 01/01/2026 até 13/07/2026': 3,
};

export type PgSamambaiaCsvRow = {
  periodStart: string;
  periodEnd: string;
  periodLabel: string;
  city: string;
...
export function buildLegacyKey(name: string, phone: string | null, address: ParsedCustomerAddress): string {
  ...
  return `legacy:pgsmb:nameaddr:${hash}`;
}

function rowFromFields(fields: string[]): PgSamambaiaCsvRow | null {
...
export function normalizeRow(row: PgSamambaiaCsvRow): NormalizedCustomer | null {
  ...
  return {
    ...
    name,
    phone,
    notes: `Migrado CSV Praia Grande Samambaia (2024-2026). ${legacyKey}`,
...
export async function loadPgSamambaiaCustomersFromCsv(filePath: string): Promise<{
  rawRows: number;
  parsedRows: number;
  skippedRows: number;
