...
export const SAO_VICENTE_PERIOD_ORDER: Record<string, number> = {
  'De 01/01/2024 até 30/06/2024': 1,
  'De 01/07/2024 até 31/12/2024': 2,
  'De 01/01/2025 até 30/06/2025': 3,
  'De 01/07/2025 até 31/12/2025': 4,
  'De 01/01/2026 até 01/07/2026': 5,
};

export type SaoVicenteCsvRow = {
  periodStart: string;
  periodEnd: string;
  periodLabel: string;
  city: string;
...
/** Fallback quando vírgulas extras deslocam colunas (ex.: nome vazio `,"",,,`). */
function parseRowLineAnchored(line: string): string[] | null {
  ...
  const phoneRaw = `(${phoneTail[1]}) ${phoneTail[2]}`;
  ...
  const metaMatch = rest.match(/,(Física),(Sem categoria|P13[^,]*)$/);
  if (!metaMatch) return null;
  const personType = metaMatch[1];
  const category = metaMatch[2];
  ...
  const cityMatch = tail.match(/^São Vicente,(.+)$/);
...
export function buildLegacyKey(name: string, phone: string | null, address: ParsedCustomerAddress): string {
  ...
  return `legacy:sv:nameaddr:${hash}`;
}

function rowFromFields(fields: string[]): SaoVicenteCsvRow | null {
...
export function normalizeRow(row: SaoVicenteCsvRow): NormalizedCustomer | null {
  const phone = normalizePhone(row.phoneRaw);
  const address = parseAddress(row.addressRaw, row.neighborhood, row.city);
  const name = resolveDisplayName(row.name, phone, address);
  if (!phone && !isUsableName(row.name.trim()) && address.street.length < 3) return null;
  ...
  return {
    ...
    name,
    phone,
    notes: `Migrado CSV São Vicente (2024-2026). ${legacyKey}`,
...
export async function loadSaoVicenteCustomersFromCsv(filePath: string): Promise<{
  rawRows: number;
  parsedRows: number;
  skippedRows: number;
