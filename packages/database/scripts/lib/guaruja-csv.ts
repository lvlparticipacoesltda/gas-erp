...
export const GUARUJA_PERIOD_ORDER: Record<string, number> = {
  'De 01/01/2024 até 31/12/2024': 1,
  'De 01/01/2025 até 31/12/2025': 2,
  'De 01/01/2026 até 08/07/2026': 3,
};

export type GuarujaCsvRow = {
  periodStart: string;
  periodEnd: string;
  periodLabel: string;
  city: string;
...
function parseRowLineAnchored(line: string): string[] | null {
  ...
  const cityMatch = tail.match(/^Guarujá,(.+)$/);
...
export function buildLegacyKey(name: string, phone: string | null, address: ParsedCustomerAddress): string {
  ...
  return `legacy:grj:nameaddr:${hash}`;
}

function rowFromFields(fields: string[]): GuarujaCsvRow | null {
...
export function normalizeRow(row: GuarujaCsvRow): NormalizedCustomer | null {
  ...
  return {
    ...
    name,
    phone,
    notes: `Migrado CSV Guarujá (2024-2026). ${legacyKey}`,
...
export async function loadGuarujaCustomersFromCsv(filePath: string): Promise<{
  rawRows: number;
  parsedRows: number;
  skippedRows: number;
