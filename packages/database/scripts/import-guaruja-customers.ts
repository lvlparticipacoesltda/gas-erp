/**
 * Importa clientes da planilha legada de Guarujá para o Gas ERP.
 *
 * Uso:
 *   pnpm --filter @gas-erp/database import:grj-customers -- --dry-run
 *   pnpm --filter @gas-erp/database import:grj-customers
 *
 * Variáveis (opcionais):
...
import { loadGuarujaCustomersFromCsv } from './lib/guaruja-csv';

const DEFAULT_ORGANIZATION_ID = 'cmqs3fqt20000jkirsp9hvdwv';
const DEFAULT_STORE_ID = 'cmr4rykcg0039qm01v4sm26dm';
const DEFAULT_CSV_PATH = path.resolve(
  __dirname,
  '../../../docs/Clientes_Guaruja_Consolidado_2024_2026.csv',
);
...
async function main() {
  ...
  const { rawRows, parsedRows, skippedRows, customers } =
    await loadGuarujaCustomersFromCsv(csvPath);
