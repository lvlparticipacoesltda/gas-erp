/**
 * Importa clientes da planilha legada de Santos para o Gas ERP.
 *
 * Uso:
 *   pnpm --filter @gas-erp/database import:sts-customers -- --dry-run
 *   pnpm --filter @gas-erp/database import:sts-customers
 *
 * Variáveis (opcionais):
...
const DEFAULT_STORE_ID = 'cmqs3fqxf0004jkirid278nw4';
const DEFAULT_CSV_PATH = path.resolve(
  __dirname,
  '../../../docs/Clientes_Santos_Consolidado_2024_2026.csv',
);
...
async function main() {
  ...
  const { rawRows, parsedRows, skippedRows, customers } =
    await loadSantosCustomersFromCsv(csvPath);
  ...
  try {
    ...
    // Carrega existentes uma vez (evita findFirst com notes contains a cada cliente).
    ...
    for (const row of existingRows) {
      const match = (row.notes ?? '').match(/legacy:sts:[^\s]+/);
      if (match) existingLegacyKeys.add(match[0]);
    }
    console.log(`Já no banco: ${existingRows.length} (phones=${existingPhones.size}, keys=${existingLegacyKeys.size})`);
